...
function ThermoPi(config) {
    ...
    app.get('/', function(req, res){
        ...
        res.redirect('/' + that.sensors.firstSensor().name + '/last24h/');
    });
        ...
        that.sensors.sensorWithName(req.params.sensor, function (sensor){
            if (sensor) {
                ...
                if (req.params.period == 'last10m') {
                    ...
                    var now = new Date(),
                        periodStartDate = now.setDate(now.getDate()-1); // last 24 hours

                    if (req.params.period == "last7d") {
                        periodStartDate = now.setDate(now.getDate()-7); // last 7 days
                    } else if (req.params.period == "last30d") {
                        periodStartDate = now.setDate(now.getDate()-30); // last 7 days
                    }
